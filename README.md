...
> 如果你还没接触 TD，但好奇大家用它做什么——这一节告诉你那些"TD 经典效果"在 Web 里对应什么。

| TD 经典案例 | Web 怎么做 |
|---|---|
| **📊 数据可视化**<br/><sub>跳动的图表、节点关系图、地理轨迹动画</sub> | [D3.js](https://d3js.org/) / [Observable Plot](https://observablehq.com/plot/) / [echarts](https://echarts.apache.org/) / [deck.gl](https://deck.gl/) |
| **✨ 粒子系统**<br/><sub>几万个点跟着鼠标、音频、流场动</sub> | [`THREE.Points`](https://threejs.org/docs/#api/en/objects/Points) + 自定义 shader / [regl](https://github.com/regl-project/regl) / [TSL](https://github.com/mrdoob/three.js/wiki/Three.js-Shading-Language) |
| **🖐️ 手势控制**<br/><sub>摄像头追手指 / 姿态，控制画面参数</sub> | [MediaPipe Web](https://developers.google.com/mediapipe/solutions/guide#web) Hands / Pose / FaceLandmarker（**和 TD 用的是同一个东西**） |
| **🎵 音画互动**<br/><sub>麦克风 / 音乐驱动视觉，频谱、节拍、振幅</sub> | Web Audio `AnalyserNode` + Three.js / p5.js，配合 [Tone.js](https://tonejs.github.io/) 做精确节拍分析 |
| **🌀 实时着色器 / VJ 视觉**<br/><sub>GLSL 程序化纹理、扭曲、辉光、Feedback 反馈循环</sub> | Three.js `ShaderMaterial` + `EffectComposer`，或者 [Hydra](https://hydra.ojack.xyz/)（浏览器原生 VJ 工具） |
| **🎭 人脸滤镜 / AR 贴纸**<br/><sub>追踪 468 个面部关键点，贴上动态遮罩</sub> | [MediaPipe FaceLandmarker](https://developers.google.com/mediapipe/solutions/vision/face_landmarker) + Three.js |
| **🎮 互动装置 / 触摸大屏**<br/><sub>触摸 / 鼠标 / 键盘 / 手柄触发动画，多点触控</sub> | 浏览器原生 `PointerEvent` + [Hammer.js](https://hammerjs.github.io/) / [Gamepad API](https://developer.mozilla.org/en-US/docs/Web/API/Gamepad_API) |
| **🎹 MIDI / OSC 控制器**<br/><sub>用 MIDI 控制器或 OSC 信号实时调参</sub> | [`webmidi.js`](https://webmidijs.org/)（浏览器直连 MIDI 设备）、[`osc-js`](https://github.com/adzialocha/osc-js)（OSC over WebSocket） |
| **📐 投影映射 / 多屏拼接**<br/><sub>把画面贴到不规则物体上，多投影仪拼接</sub> | Three.js `OrthographicCamera` + UV 变形，或者 [maptastic.js](https://github.com/glowbox/maptasticjs) |
| **🎬 生成式动画 / 程序化图形**<br/><sub>用噪声、L-system、流场生成"会自己动"的图形</sub> | [p5.js](https://p5js.org/) 起步最快，进阶用 Three.js + GLSL |

> [!NOTE]
> 看完这些有没有发现：**你能想到的 TD 用法，Web 都有现成的库做对应的事**。区别只是——Web 的方案是开放的、跨平台的、AI 能直接帮你写的。
