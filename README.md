...
## 一份开源指南：TD 常用节点的 Web 替代方案速查。你不需要自己读完它——把这个文档扔给 AI Agent，让它帮你查、帮你写。附带三个可在线体验的 Demo。

**作者：** [sjwwhenever](https://sjwwhenever.com) ｜ **小红书：** 不可兼容
...
最近 TD 有了 MCP（Model Context Protocol）的支持，看起来是在拥抱 AI。但仔细看，**那是社区第三方做的，不是官方的**。这说明什么？说明官方在 AI 这件事上并没有真正主动推进。MCP 不是长久之计——如果官方不从根本上开放 TD 的架构和 API，光靠社区补丁是无法逆转趋势的。

## 一个很能说明问题的例子：MediaPipe

在 TD 圈子里，MediaPipe（人脸追踪、手势识别、姿态检测）是影响力很大的一个能力。很多人觉得这是 TD 的功能。

但真相是：**TD 的 MediaPipe 插件底层就是启动了一个 Chromium 浏览器，跑的是 WebAssembly + WebGL，再通过 WebSocket 把结果传回 TD**。它本质上就是套壳了一个网页应用。
...
- **AI 帮你写代码**。你描述你要什么效果，AI 写出来。你的角色是导演，不是程序员。
- **浏览器原生能力已经爆炸**。Web Audio、WebGL、WebGPU、WebMIDI、WebSocket、摄像头——这些全是浏览器自带的，不需要任何插件。
- **Web 是开放的**。任何人可以用，任何 AI 可以读写，任何平台可以跑。没有授权费，没有分辨率限制，没有 `.toe` 文件只有装了 TD 的人才能打开的问题。
- **部署成本是零**。一条链接，全世界任何人在任何设备上打开就能用。

更关键的是：**Web 的开放性和 AI 是天然互补的。** AI 能读所有 Web 文档、能生成所有 Web 代码、能调试所有 Web 错误。而 TD？AI 连它的节点参数文档都不一定能找全。
...
1. **TD 节点 → Web 替代方案的速查表**（下面的映射表）——AI 一查就知道该用什么库
2. **给 AI 的提示词模版**——让 AI 直接帮你用 Web 实现你想要的效果
3. **三个可以直接跑的例子**——你可以让 AI 参考着改
...
最简单的方式：**把这个仓库给你的 AI Agent 当参考资料。** 比如你可以跟 AI 说：

```
参考这个仓库 [仓库链接]，我想在网页里实现 [描述你想要的效果]。
```

AI 会自己去查映射表、找对应的库、参考 Demo 代码，然后帮你写出来。

如果你想更精确地控制 AI 的输出，可以用这个模版：
...
| # | 目录 | 技术栈 | 难度 | 涉及 TD 能力 |
|---|---|---|---|---|
| 1 | [`demos/01-vanilla-audio-reactive/`](./demos/01-vanilla-audio-reactive/) | 纯 HTML + p5.js + Web Audio | 零基础 | CHOP（麦克风/频谱） |
| 2 | [`demos/02-vite-ts-shader-feedback/`](./demos/02-vite-ts-shader-feedback/) | Vite + TypeScript + Three.js + GLSL | 中级 | TOP（Feedback + GLSL） |
| 3 | [`demos/03-svelte-midi-playground/`](./demos/03-svelte-midi-playground/) | SvelteKit + Three.js + webmidi.js | 熟练 | MIDI + SOP + TOP |
