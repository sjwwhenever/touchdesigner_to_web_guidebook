...
**📊 数据可视化**
跳动的图表、节点关系图、地理轨迹动画。
→ Web 里用 [D3.js](https://d3js.org/) / [Observable Plot](https://observablehq.com/plot/) / [echarts](https://echarts.apache.org/) / [deck.gl](https://deck.gl/)。
...
**✨ 粒子系统**
几万个点跟着鼠标、音频、流场动。
→ [`THREE.Points`](https://threejs.org/docs/#api/en/objects/Points) + 自定义 shader / [regl](https://github.com/regl-project/regl) / [TSL](https://github.com/mrdoob/three.js/wiki/Three.js-Shading-Language)。
...
**🖐️ 手势控制**
摄像头追手指 / 姿态，控制画面参数。
→ [MediaPipe Web](https://developers.google.com/mediapipe/solutions/guide#web) Hands / Pose / FaceLandmarker，**和 TD 用的是同一个东西**。
...
**🎵 音画互动**
麦克风/音乐驱动视觉，频谱、节拍、振幅。
→ Web Audio `AnalyserNode` + Three.js / p5.js，配合 [Tone.js](https://tonejs.github.io/) 做更精确的节拍分析。
...
**🌀 实时着色器 / VJ 视觉**
GLSL 写的程序化纹理、扭曲、辉光、Feedback 反馈循环。
→ Three.js `ShaderMaterial` + `EffectComposer`，或者直接用 [Hydra](https://hydra.ojack.xyz/)（浏览器原生的 VJ 工具）。

</td>
<td width="50%">

**🎭 人脸滤镜 / AR 贴纸**
追踪 468 个面部关键点，贴上动态遮罩。
→ MediaPipe FaceLandmarker + Three.js，可以直接套用 TD MediaPipe 插件能做的所有效果。
...
**🎮 互动装置 / 触摸大屏**
触摸 / 鼠标 / 键盘 / 手柄触发动画，多点触控。
→ 浏览器原生 `PointerEvent` + [Hammer.js](https://hammerjs.github.io/) / [Gamepad API](https://developer.mozilla.org/en-US/docs/Web/API/Gamepad_API)。

</td>
<td width="50%">

**🎹 MIDI / OSC 控制器**
用 MIDI 控制器或 OSC 信号实时调参。
→ [`webmidi.js`](https://webmidijs.org/)（浏览器直连 MIDI 设备）、[`osc-js`](https://github.com/adzialocha/osc-js)（OSC over WebSocket）。
...
**📐 投影映射 / 多屏拼接**
把画面贴到不规则物体上，多投影仪拼接。
→ Three.js 的 `OrthographicCamera` + UV 变形，或者 [maptastic.js](https://github.com/glowbox/maptasticjs)。

</td>
<td width="50%">

**🎬 生成式动画 / 程序化图形**
用噪声、L-system、流场生成"会自己动"的图形。
→ [p5.js](https://p5js.org/) 起步最快，进阶用 Three.js + GLSL。
